...
[s1 s2]=size(p);
if(s1==2)
    p(3,:)=0;   %convert 2D to 3D
end

%Eliminate unnecessary triangles
Remove=find(t(4,:)>2); %Here's the difference of Chapter 10  
t(:,Remove)=[];           
...
for m=1:TrianglesTotal
    ...
    for k=m+1:TrianglesTotal
        ...
        if(sum(a)==2) %triangles m and k have two common points
            n=n+1;
...
save mesh1  p ...
            ...
            Area ...
            Center ...
            h               %This is new (patch height)

%View result (make sure everything is ok)
cd mesh
viewer('patch')
cd ..            